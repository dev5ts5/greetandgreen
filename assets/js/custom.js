...
var input_feilds_html = "";
  ...
  jQuery.ajax({
    ...
    success: function(result){
      ...
      // // console.log(result);
      result = result.slice(0, -1);
      shop_details = result.split('$%^&*$')[0];

      console.log("HELLO HELLO"+result.split('$%^&*$')[1]);
      jQuery('.fpd-main-wrapper').css({'cssText':'background-size: cover !important;background-repeat: no-repeat !important;background-image:url("'+result.split('$%^&*$')[1]+'") !important;background-position: center!important;'});
      console.log("HELLO HELLO"+result.split('$%^&*$')[1]);
...
function opennext_tab(evt, cityName) {
  ...
  if(cityName == 'musique')
  {
    ...
    jQuery('.fpd-module').removeClass('fpd-active')
...
jQuery(document).on('click','.top_button',function(e)
{
  ...
  if( jQuery('#animation_header_carte').hasClass('active') )
  {
...
jQuery('body').append('<div class="popup_choose_envelope" id="choose_card_envelope_option"> <div class="popup-header"><h2 class="popup-title">Choisissez entre ces</h2><span onclick="close_choose_animation()" class="close-icon closed"></span></div> <div id="inner_choose_envelope_option" > <div onclick="select_type_main('+"'clips'"+')" id="choose_enve_no" > <img src="" id="card_image_choose" /> </div> <div onclick="select_type_main('+"'cartes'"+')" id="choose_enve_yes" > <img src="" id="envelope_image_choose" /> <img src="" id="card_image_2_choose" /> </div> </div> </div>');

function close_choose_animation()
{
  jQuery('#choose_card_envelope_option').removeClass('active');
  jQuery('.overlay').removeClass('active');
}
...
function hide_animation_content_popup()
{
  const e1 = document.getElementById("radio_for_choose_ani");
    if(!(e1 == null))
      e1.remove();
}